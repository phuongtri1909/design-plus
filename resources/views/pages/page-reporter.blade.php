                ...
                <div id="formReporter" class="form-reporter">
                    <div class="form-post">
                        ...
                        <form id="myForm" action="#" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group row mb-2">
                                <div class="col-12">
                                    <input id="title" type="text"
                                        ...
                                        value="{{ old('title') ?? ($post_draft->title ?? '') }}" autocomplete="title"
                            ...
                            <div class="form-group row mb-2">
                                <div class="col-12">
                                    <select id="post_type" class="form-select @error('post_type') is-invalid @enderror"
                                        name="post_type">
                                        <option value="" disabled selected>Loại tin</option>
                                        <option value="new"
                                            {{ (old('post_type') ?? ($post_draft->post_type ?? '')) == 'new' ? 'selected' : '' }}>
                                            viết mới</option>
                                        <option value="translation"
                                            {{ (old('post_type') ?? ($post_draft->post_type ?? '')) == 'translation' ? 'selected' : '' }}>
                                            Bài dịch</option>
                                    </select>
                                        ...
                                        @if ($categorys)
                                            @foreach ($categorys as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ (old('category_id') ?? ($post_draft->category_id ?? '')) == $category->id ? 'selected' : '' }}>
                            ...
                            <div class="form-group row mb-2">
                                <div class="col-12">
                                    <textarea rows="5" id="brief_intro" class="form-control @error('brief_intro') is-invalid @enderror"
                                        name="brief_intro" autocomplete="brief_intro" placeholder="Giới thiệu ngắn về bài viết">{{ old('brief_intro') ?? ($post_draft->brief_intro ?? '') }}</textarea>
                            ...
                            <div class="form-group row mb-2">
                                <div class="col-12">
                                    <textarea rows="10" id="content" class="form-control @error('content') is-invalid @enderror" name="content"
                                        autocomplete="content" placeholder="Nội dung bài viết">{{ old('content') ?? ($post_draft->content ?? '') }}</textarea>
                                ...
                                <div class="col-12">
                                    <p class="text-center">
                                        <label class="label-uploade" for="attachment">
                                            <a class="btn-upload form-control" role="button" aria-disabled="false">Upload
                            ...
                            <div class="btn-action mb-2 mb-5 text-center">
                                <div class="row">
                                    <div class="col-12 col-xl-4 mb-2">
                                        <button id="btn-review" type="button" data-bs-toggle="modal"
                                            data-bs-target="#GFG" class="btn-border-gradient border-gradient-blue">
                                            Xem lại bài
                                        </button>
                                    </div>
                                    <div class="col-12 col-xl-4 mb-2">
